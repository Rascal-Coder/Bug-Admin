...
import { Cursor, CursorBody, CursorMessage, CursorName, CursorPointer } from "@/components/cursor";
	...
	return (
		<div className="relative aspect-[4/3] size-full bg-[radial-gradient(var(--color-secondary),transparent_1px)] [background-size:16px_16px]">
			...
			{usersWithPositions.map((user, index) => (
				...
				>
					<CursorPointer className={cn(colors[index % colors.length].foreground)} />
					<CursorBody
						className={cn(
					...
					>
						<div className="flex items-center gap-2 opacity-100!">
							<img alt={user.name} className="mt-0 mb-0 size-4 rounded-full" height={16} src={user.avatar} width={16} />
