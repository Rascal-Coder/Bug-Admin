...
const LineLoadingItem = styled.div<{ $themeMode: string }>`
	background-color: ${({ $themeMode }) => ($themeMode === "light" ? commonColors.black : commonColors.white)};
	...
	return (
		<div className="flex h-full w-full flex-col items-center justify-center">
			<LineLoadingWrapper className="relative h-1.5 w-96 overflow-hidden rounded">
				<LineLoadingItem className="absolute left-0 top-0 h-full w-1/3" $themeMode={themeMode} />
			</LineLoadingWrapper>
		</div>
	);
}
